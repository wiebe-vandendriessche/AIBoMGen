...
const CardDecorator = ({ children }: { children: ReactNode }) => (
    <div className="relative mx-auto size-36 duration-200 [--color-border:color-mix(in_oklab,var(--color-zinc-950)30%,transparent)] group-hover:[--color-border:color-mix(in_oklab,var(--color-zinc-950)60%,transparent)] dark:[--color-border:color-mix(in_oklab,var(--color-white)40%,transparent)] dark:group-hover:bg-white/20 dark:group-hover:[--color-border:color-mix(in_oklab,var(--color-white)70%,transparent)]">
        <div aria-hidden className="absolute inset-0 bg-[linear-gradient(to_right,var(--color-border)_1px,transparent_1px),linear-gradient(to_bottom,var(--color-border)_1px,transparent_1px)] bg-[size:24px_24px]" />
        <div aria-hidden className="bg-radial to-background absolute inset-0 from-transparent to-75%" />
        <div className="absolute inset-0 m-auto flex size-12 items-center justify-center border-l border-t bg-card">
            {children}
        </div>
    </div>
)
